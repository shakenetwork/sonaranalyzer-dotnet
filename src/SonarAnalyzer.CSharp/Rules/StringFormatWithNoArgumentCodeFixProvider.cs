        ...
        protected sealed override async Task RegisterCodeFixesAsync(SyntaxNode root, CodeFixContext context)
        {
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    ...
                    c =>
                    {
                        var formatStringIndex = int.Parse(diagnostic.Properties[StringFormatWithNoArgument.FormatStringIndexKey]);
                        var newRoot = root.ReplaceNode(
                            invocation,
                            invocation.ArgumentList.Arguments[formatStringIndex].Expression.WithTriviaFrom(invocation));
