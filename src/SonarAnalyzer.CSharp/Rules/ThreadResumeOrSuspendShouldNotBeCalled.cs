...
namespace SonarAnalyzer.Rules.CSharp
{
    ...
    [Rule(DiagnosticId)]
    public class ThreadResumeOrSuspendShouldNotBeCalled : SonarDiagnosticAnalyzer
    {
        internal const string DiagnosticId = "S3889";
        internal const string MessageFormat = "Refactor the code to remove this use of '{0}'";
        private static readonly HashSet<string> InvalidIdentifiers = new HashSet<string>
        {
            "Suspend",
            "Resume"
        };
        ...
        private void CheckForIssue(SyntaxNodeAnalysisContext analysisContext)
        {
            ...
            if (call == null || !InvalidIdentifiers.Contains(call.Name.Identifier.ValueText))
            {
                return;
            }
            ...
            if (!methodCallSymbol.Symbol.ContainingType.ConstructedFrom.Is(KnownType.System_Threading_Thread))
            {
                return;
            }
